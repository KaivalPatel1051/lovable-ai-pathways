...
export interface BentoProps {
  ...
  enableTilt?: boolean;
  glowColor?: string; // RGB string like "132, 0, 255"
  clickEffect?: boolean;
...
// Updated to teal/cyan accent for dark palette
const DEFAULT_GLOW_COLOR = "0, 214, 199"; // teal/cyan
const MOBILE_BREAKPOINT = 768;

// Exportable dark palette for consistent use
export const DarkPalette = {
  background: "#0b1220", // charcoal/navy
  surface: "#0f172a", // slightly lighter surface
  glowRGB: DEFAULT_GLOW_COLOR, // for rgba() uses
  accentHex: "#00d6c7", // teal/cyan hex
};

const cardData: BentoCardProps[] = [
  { color: "#0b1220", title: "Analytics", description: "Track user behavior", label: "Insights" },
  { color: "#0b1220", title: "Dashboard", description: "Centralized data view", label: "Overview" },
  { color: "#0b1220", title: "Collaboration", description: "Work together seamlessly", label: "Teamwork" },
  { color: "#0b1220", title: "Automation", description: "Streamline workflows", label: "Efficiency" },
  { color: "#0b1220", title: "Integration", description: "Connect favorite tools", label: "Connectivity" },
  { color: "#0b1220", title: "Security", description: "Enterprise-grade protection", label: "Protection" },
];
...
const ParticleCard: React.FC<{
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (enableMagnetism) {
        const magnetX = (x - centerX) * 0.02; // gentler magnetism
...
const BentoCardGrid: React.FC<{ children: React.ReactNode; gridRef?: React.RefObject<HTMLDivElement | null> }>
  = ({ children, gridRef }) => <div className="card-grid bento-section" ref={gridRef}>{children}</div>;
    ...
    <>
      ...
      <BentoCardGrid gridRef={gridRef}>
        {cardData.map((card, index) => {
          ...
          if (enableStars) {
            return (
              <ParticleCard key={index} {...cardProps} disableAnimations={shouldDisable} particleCount={particleCount} glowColor={glowColor} enableTilt={enableTilt} clickEffect={clickEffect} enableMagnetism={enableMagnetism}>
...
// Reusable wrapper for applying MagicBento particle/tilt/magnetism effects to arbitrary content
export { ParticleCard as MagicBentoCard };
